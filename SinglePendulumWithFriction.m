% Simple Pendulum Animation with Friction and GIF Generation
clear all; close all; clc;

% Parameters
g = 9.81;           % Gravitational acceleration (m/s^2)
L = 1;              % Length of the pendulum (m)
b = 0.2;            % Damping coefficient (s^-1, adjust for more/less friction)
...
% Differential equation for the pendulum with viscous damping
ode = @(t, y) [y(2); -b*y(2) - (g/L)*sin(y(1))]; % y(1) = theta, y(2) = dtheta/dt
...
filename = 'pendulum_with_friction_animation.gif';
...
for i = 1:length(t_anim)
    ...
    xlabel('X (m)');
    ylabel('Y (m)');
    title('Simple Pendulum with Friction Animation');
    grid on;
